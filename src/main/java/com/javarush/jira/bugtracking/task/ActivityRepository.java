...
public interface ActivityRepository extends BaseRepository<Activity> {
    ...
    @Query("SELECT a FROM Activity a JOIN FETCH a.author WHERE a.taskId =:taskId ORDER BY a.updated ASC")
    List<Activity> findAllByTaskIdOrderByUpdatedAsc(long taskId);
